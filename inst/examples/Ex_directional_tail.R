...
#arrows(delta[1], delta[2],
#       delta[1] + w[1] * 2, delta[2] + w[2] * 2,
...
r <- sqrt(sum(delta^2))
symbols(delta[1], delta[2], circles = r, inches = FALSE, add = TRUE, lwd = 2, border = "gray")
...
# Add legend
legend("topright",
       legend = c("Tail draws", "Non-tail draws", "Direction vector", "Radius boundary",
                  "Prior mean", "Posterior mode"),
...
plot(B, col = ifelse(flag, "red", "blue"), pch = 19,
     xlab = "Intercept", ylab = "groupTrt",
...
legend("topright", legend = c("Tail draws", "Non-tail draws", "Prior", "Posterior"),
       col = c("red", "blue", "black", "darkgreen"), pch = c(19, 19, 4, 3))
