// @source_type: c
// @source_origin: bessel_y_cycle_free.c
// @includes: nmath.h, bessel.h, refactored.h, Memory.h
// @depends: cospi, nmath, bessel, refactored
// @provides: bessel_y_cycle_free, bessel_y_cycle_free_ex
// @all_depends_count: 5
// @all_depends: bessel, refactored, Rmath, nmath, cospi
// @load_order: 71
// @local_macros: min0
...
// openclport-disabled-include: #include <R_ext/Memory.h>
...
/* cycle-free variant: supports alpha >= 0 and excludes cross-calls */
double bessel_y_cycle_free(double x, double alpha)
{
    int nb, ncalc;
    double na, *by;
    const void *vmax;
...
#endif
    if (x < 0) {
        ML_WARNING(ME_RANGE, "bessel_y_cycle_free");
        return ML_NAN;
    }
    na = floor(alpha);
    if (alpha < 0) {
...
/* cycle-free variant with caller-provided workspace */
double bessel_y_cycle_free_ex(double x, double alpha, double *by)
{
    int nb, ncalc;
    double na;
...
#endif
    if (x < 0) {
        ML_WARNING(ME_RANGE, "bessel_y_cycle_free_ex");
        return ML_NAN;
    }
    na = floor(alpha);
    if (alpha < 0) {
