...
#ifndef MAYBE_R_CheckUserInterrupt
# define MAYBE_R_CheckUserInterrupt() R_CheckUserInterrupt()
#endif

/* Template-instantiated search body:
   emit only when caller has defined distribution-specific macros. */
#if defined(_thisDIST_) && defined(_dist_PARS_DECL_) && defined(_dist_PARS_)
static double DO_SEARCH_FUN(_dist_PARS_DECL_)
{
	...
	for(int iter = 0; ; iter++) {
	    double newz = -1.; // -Wall
	    if(iter % 10000 == 0) MAYBE_R_CheckUserInterrupt();// have seen inf.loops
	    if(y > 0)
...
#endif /* distribution-specific template instantiation guards */
