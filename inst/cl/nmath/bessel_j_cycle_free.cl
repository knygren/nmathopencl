// @source_type: c
// @source_origin: bessel_j_cycle_free.c
// @includes: nmath.h, bessel.h, refactored.h, Memory.h
// @depends: fmax2, gamma_cody, nmath, bessel, refactored
...
double bessel_j_cycle_free(double x, double alpha)
{
    int nb, ncalc;
    double na, *bj;
    const void *vmax;
...
#endif
    if (x < 0) {
        ML_WARNING(ME_RANGE, "bessel_j_cycle_free");
        return ML_NAN;
    }
    na = floor(alpha);
    if (alpha < 0) {
...
double bessel_j_cycle_free_ex(double x, double alpha, double *bj)
{
    int nb, ncalc;
    double na;
...
#endif
    if (x < 0) {
        ML_WARNING(ME_RANGE, "bessel_j_cycle_free_ex");
        return ML_NAN;
    }
    na = floor(alpha);
    if (alpha < 0) {
