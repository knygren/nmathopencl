...
#' @param q Numeric vector of quantiles for \code{pcauchy_opencl}; recycled like \code{stats::pcauchy}.
#' @param p Numeric scalar probability in \code{[0, 1]}.
#' @param location Location parameter.
#' @param scale Scale parameter (must be > 0).
#' @param fallback Logical; if \code{TRUE}, fall back to CPU behavior on OpenCL error.
#' @param verbose Logical; print fallback/error diagnostics.
#' @param lower.tail,log.p As in \code{stats::pcauchy} for \code{pcauchy_opencl} (vector inputs recycled).
#' @param opencl_parallel OpenCL dispatch hint for \code{pcauchy_opencl} (\code{TRUE}, \code{FALSE}, or \code{NA}); reserved for future parallel kernels.
#' @param log Logical; if \code{TRUE}, return log-density for \code{dcauchy_opencl} (like \code{\link[stats]{dcauchy}}).
#'
