...
#'   \code{pf_opencl()} and \code{qf_opencl()}.
...
#' @rdname f_opencl
#' @export
df_opencl <- function(n, x, df1, df2, fallback = TRUE, verbose = FALSE) {
  ...
  .opencl_try_or_fallback(
    opencl_expr = function() .df_opencl(n, x, df1, df2, verbose = verbose),
    fallback_expr = function() rep(stats::df(x, df1 = df1, df2 = df2), n),
