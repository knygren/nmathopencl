...
example("directional_tail", package = "nmathopencl")
...
Increasing $v$ makes the Studentâ€‘t closer to normal (lighter tails), so the tail probability
...
Either effect reduces the denominator and can increase the raw $t$ value â€” but combined with larger $\boldsymbol{v}$ the net tail probability can still be smaller.
...
- If the prior pulls $\boldsymbol{m_{n,j}}$ toward $\boldsymbol{\beta_{0}}$ (e.g., 0), 
  ...
  Griffin & Brown and other shrinkageâ€‘prior literature discuss this tradeoff 
  explicitly for normalâ€“gamma shrinkage priors.
