...
The user supplies two functions â€” `f2` for the **negative** log-posterior and `f3` for the corresponding gradient â€” which are passed to the `optim()` routine.
...
- The prior varianceâ€“covariance matrix is the identity matrix.  
...
(v) **Remark 14**: Suppose that a model has a multivariate normal prior with a diagonal varianceâ€“covariance matrix. Also assume that the posterior precision at the posterior mode is a diagonal matrix. Then the model can be reparameterized into a model in standard form.
...
### Deep Dive: Gridtype 1 â€” Static Threshold Envelope
...
### Deep Dive: Gridtype 2 â€” Adaptive Envelope via `EnvelopeOpt()`
...
The function `EnvelopeOpt(a_i, n)` solves this tradeoff using approximations from subgradient theory. Dimensions are ranked by decreasing posterior standard deviation â€” equivalently, by increasing values of \( 1 / (1 + a_i) \). Dimensions with larger `a_i` (i.e., sharper curvature) are more likely to benefit from additional tangents.
...
represents the **grid construction cost** at iteration *i*. This term estimates the number of logâ€‘likelihood and gradient evaluations required to build the envelope as more dimensions are promoted from singleâ€‘point to threeâ€‘point tangents:

- At the start ($i = 1$), all dimensions use singleâ€‘point tangents, so the build cost is minimal.  
- Each time a new dimension is switched to a threeâ€‘point tangent, the number of required evaluations grows multiplicatively.  
- The factor $3^{\,i-1}$ captures this exponential growth: with each additional threeâ€‘point dimension, the grid size triples, and so does the number of evaluations needed to construct the envelope.
...
- At initialization, `scaleest[1,] <- sqrt(1 + a1)`, so  
  ...
  \]  
  which corresponds to the baseline rejection burden when all dimensions use singleâ€‘point tangents.  

- Each time a dimension is promoted to a threeâ€‘point tangent, its entry in `scaleest` is replaced by the constant  
  \[
  \tfrac{2}{\sqrt{\pi}},
  \]  
  reflecting the improved acceptance bound for that dimension.  

- Consequently, as more dimensions are switched to threeâ€‘point tangents, the product  
  \[
...
- **Large** `slopeest` values â†’ more rejections, higher perâ€‘draw cost.  
- **Smaller** `slopeest` values â†’ fewer rejections, more efficient sampling.  
...
- For **small sample sizes** ($n$ small), the build cost dominates, so the algorithm favors fewer threeâ€‘point tangents.  
...
The implementation of `EnvelopeBuild` follows the envelope construction in Nygren & Nygren (2006) for models in standard form (see Section 3â€“3.3). The outline below captures the process when each dimension involves three-point tangents. If some dimensions involve only one-point tangents, only the mid-point is used and the intervals become the full real-number line. 
...
This step constructs the likelihood subgradient densities and facilitates acceptâ€“reject sampling. The subgradients \( c(\bar{\theta}) \) are part of the definitions of likelihood subgradient densities (see Definition 2). Both subgradients and negative log-likelihoods (via \( h_{\bar{\theta}}(.) \)) are used during the acceptâ€“reject procedure.
...
The envelope provides a mixture of restricted likelihoodâ€“subgradient densities, each defined over a region \( A_i \), with associated mixture weights \( \tilde{p}_i \) stored in `PLSD`. The sampling proceeds as follows:
...
- **coefficients**: n Ã— k matrix of posterior draws on the original parameter scale (returned as `trans(out2)` in the C++ routing function).  
