...
vignette: >
  %\VignetteIndexEntry{Chapter A01: A detailed overview of the glmbayes package}
...
For users who are interested in the details of how the simulation is performed there is the final section that contains a discussion of the various simulation functions that are used to sample from the posterior distributions.  This includes a discussion of three core function that simulate for the currently implemented prior families and a couple of functions related to what we call the Central Normal distribution (the normal distribution between a lower and an upper bound). It also includes a discussion of a set of functions that are used in order to utilize the enveloping functions needed to get iid samples for some of the posterior distributions. That material is rather technical; the likelihood-subgradient foundation is given by [@Nygren2006], with estimation architecture summarized in [@glmbayesChapterA02] and envelope details in [@glmbayesChapterA08]. 


| Function Type | Functions | Vignettes |
|---------------|-----------|-----------|
| **Prior Setup function**| **Prior_Setup** | Chapter 01 â€” Getting Started with glmbayes  |
|**Main Model Interfaces (GLM / LM)**|**glmb** | Chapter 01 â€” Getting Started with glmbayes |
|**Methods**|**print.glmb** | Chapter 01 â€” Getting Started with glmbayes |
| **pfamilies** | **dNormal**|Chapter 02 â€” Estimating Bayesian Linear Models|
| **Main Model Interfaces (GLM / LM)**| **lmb**  |Chapter 02 â€” Estimating Bayesian Linear Models |
| **Methods**| **summary.lmb**  |Chapter 02 â€” Estimating Bayesian Linear Models |
| **Prior Setup function**| **Prior_Setup** | Chapter 03 â€” Tailoring Priors - Leveraging the Prior_Setup Function; Chapter A12 â€” Technical Derivations for Priors Returned by Prior_Setup  |
| **Methods** | **print.PriorSetup** | Chapter 03 - Tailoring Priors - Leveraging the Prior_Setup Function  |
| **Methods**| **predict.glmb, residuals.glmb, extractAIC.glmb** | Chapter 04 â€” Predictions, Deviance Residuals, Model Statistics |
|**Main Model Interfaces (GLM / LM)**|**glmb** | Chapter 06 â€” Estimating Bayesian Generalized Linear Models |
|**Methods**|**summary.glmb** | Chapter 06 â€” Estimating Bayesian Generalized Linear Models |
|**Main Model Interfaces (GLM / LM)**|**glmb** | Chapter 07 â€” Models for the Binomial Family|
|**Main Model Interfaces (GLM / LM)**|**glmb** | Chapter 08 â€” Models for the Poisson Family |
|**Main Model Interfaces (GLM / LM)**|**glmb** | Chapter 09 â€” Models for the Gamma Family |
| **Lower-Level Interfaces (GLM / LM)** | **rlmb** | Chapter 13 â€” Hierarchical Linear Models |
| **Lower-Level Interfaces (GLM / LM)** | **rglmb** | Chapter 14 â€” Hierarchical Generalized Linear Models |
...
| **pfamilies** |**dGamma, dNormal_Gamma, dIndependent_Normal_Gamma** | Chapter 11 â€” Estimating Models with Unknown Dispersion Parameters |
| **Methods**| **Various method functions for glmbayes** | Chapter A03 â€” Methods Available in glmbayes |
| **Envelope Related Functions** | Various Functions | Chapter A08 â€” Overview of Envelope Related Functions |
...
| **Utilities** | add_to_libpath_linux, add_to_path, add_to_path_linux, add_to_path_windows| Chapter 00 â€” Introduction<br>Chapter 01 â€” Getting Started |
| **Datasets** | AMI, carinsca, Cleveland | Chapter 01 â€” Getting Started |
...
To access information for the package outside of the information contained in these vignettes, we suggest users review three sets of help pages. A call to help(nmathopencl) yields a page with a detailed discussion of the package with an example at the bottom.   Calling help(package = "nmathopencl") brings the user to a list of Help pages for specific functions as well as links to a general description file, code demos, and these vignettes. Part of the objective of this vignette is to provide more organized layout of the functions than the general list of help pages provided on the documentation page.

A call to demo(package = "nmathopencl"), finally, brings up a list of available demos for the package. A useful start for users might be to go through the specific demos as they are set up to illustrate the functionality of the package.

* help(nmathopencl)
* help(package = "nmathopencl")
* demo(package = "nmathopencl")
...
This function can be used to set up the prior arguments so they have the correct dimensions and so that the various dimensions have names that correspond to those in the model. In addition to initialized prior structures, these functions also return the model frame and the model matrix as a reference (the latter when it was requested from the underlying model). It is worth noting that this function is intended as a tool for initializing the prior objects and not for setting them to the final values used in the estimation. How to populate the arguments with sensible values is discussed in a separate vignette.

NOTE: ADD A PRINT METHOD FOR THIS FUNCTION SO THAT THE OUTPUT CAN BE CONTROLLED

```{r Calling Prior_Setup,results = "hide"}
Prior_Setup(weight ~ group,family=gaussian())
```
...
* The *rNormal_reg* function is used for the *dNormal* *pfamily*
* The *rGamma_reg* function is used for the *dGamma* *pfamily*
* The *rNormalGamma_reg* function is used for the *dNormal_Gamma* *pfamily*
...
* *EnvelopeSetGrid* calculates log-densities associated with the restricted normals used to generate candidates
* *EnvelopeSetLogP* calculates constants used during sampling from the constructed envelope
* *rNormalGLM_std* simulates from a standardized Bayesian Generalized linear model using an Enveloping function
