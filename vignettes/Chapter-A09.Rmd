...
| `rIndepNormalGammaReg_cpp` | Gaussian with independent Normalâ€“Gamma prior, `use_parallel = TRUE` and `n > 1` | Uses `rIndepNormalGammaReg_std_parallel_cpp` |
...
1. **Pilot run** â€“ A small serial or parallel batch (e.g., 1 draw or a few draws) to measure per-draw or per-observation time.
2. **Calibration run** â€“ A larger parallel batch (typically up to ~1% of `n`, capped so it takes at most ~5 minutes) to refine the time estimate.
3. **Time estimate** â€“ Extrapolation to the full sample size `n`.
4. **Interactive safeguard** â€“ If the estimate exceeds 5 minutes, the user is prompted in interactive sessions.
5. **Non-interactive behavior** â€“ In non-interactive sessions (e.g., R CMD check, batch jobs), the run proceeds automatically without prompting.
...
**Responses:** `y`, `yes`, `1`, or `continue` â†’ proceed; `n`, `no`, `2`, or empty â†’ stop with an informative error.
...
- `est_per_draw_sec` = `per_candidate_sec` Ã— `E_draws` (expected candidates per accepted draw)
- `est_total_sec` = `est_per_draw_sec` Ã— `n`
...
For Gaussian regression with independent Normalâ€“Gamma priors, the pilot logic is similar in spirit but tailored to joint \((\beta, \phi)\) sampling:
...
|----------|----------------|
| **Debugging or small runs** | `use_parallel = FALSE` â€“ serial path is interruptible |
| **Large production runs** | `use_parallel = TRUE` â€“ faster; rely on the pilot and prompt |
...
- **RcppParallel** â€“ Provides TBB and the `parallelFor` interface
- **TBB** â€“ Bundled with RcppParallel; no separate installation required
...
- **Chapter A05** â€“ Simulation pipeline (optimization, standardization, envelope, sampling)
- **Chapter A08** â€“ Envelope-related functions
- **Chapter A10** â€“ OpenCL acceleration for envelope construction
- **`?glmb`**, **`?rglmb`**, **`?lmb`**, **`?rlmb`** â€“ `use_parallel` argument
- **NEWS.md** â€“ Parallel sampling and pilot-related changes
