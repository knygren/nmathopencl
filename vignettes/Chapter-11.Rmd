...
and for quasiâ€‘families it corresponds to fixing the scale parameter.
...
(2) Joint conjugate Normalâ€“Gamma priors  
(3) Independent Normalâ€“Gamma priors  
      â€¢ iid sampling via truncated Gamma priors  
      â€¢ twoâ€‘block Gibbs sampling  

Full derivations for Gaussian dispersion, shape, and rate calibration from `Prior_Setup()` / `compute_gaussian_prior()`â€”and how those objects connect to conjugate and independent Normal--Gamma priorsâ€”are given in **Chapter A12** (<https://knygren.r-universe.dev/articles/glmbayes/Chapter-A12.html>).

All Gaussian examples use the **Dobson** plantâ€‘weight data [@Dobson1990], first introduced in Chapter 02.
...
- valid acceptâ€“reject sampling for \(\tau\)  


### Implications for Bayesian GLMs

The Gamma logâ€“link model thus shares the same structural advantages as the
...
* compatibility with twoâ€“block Gibbs sampling when desired.

These properties make the Gamma logâ€“link model an especially clean example of
how exponentialâ€“family structure supports fast, reliable posterior simulation in
glmbayes.
...
## 3.2 Joint Conjugate Normalâ€“Gamma Prior
...
## 3.3 Independent Normalâ€“Gamma Prior
...
A variation on this prior specificationâ€”using a **truncated Gamma prior** for the
dispersionâ€”is the model developed in **Chapter A07**, where:

- the conditional posterior in \( \beta \) is *not conjugate*,  
- the conditional posterior in \( \tau = 1/\phi \) is *not conjugate*, and  
- the joint posterior remains **logâ€‘concave**, enabling  
  the **acceptâ€“reject envelope sampler**.

This truncatedâ€‘Gamma, nonâ€‘conjugate Normalâ€“Gamma model is therefore a direct
extension of the independent Normalâ€“Gamma prior described above, but requires
the more advanced envelopeâ€‘based sampling strategy documented in Chapterâ€¯A07.
...
Under default settings, both the truncatedâ€‘Gamma iid sampler and the twoâ€‘block
...
* the iid sampler using the independent Normalâ€“Gamma prior (same specification
  as `lmb(..., dIndependent_Normal_Gamma(...), n = 10000)` above), and  
* the twoâ€‘block Gibbs sampler constructed from alternating Normal and Gamma updates.
...
* **No tuning or burnâ€‘in**: each draw is a direct sample from the posterior
...
acceptâ€‘reject algorithm for the independent Normalâ€“Gamma prior.
...
We now specify priors for \((\beta,\tau)\) and derive the corresponding
logâ€‘priors and logâ€‘posteriors, using the **carinsca** data [@BaileySimon1960; @statsciCarinsca; @McCullagh1989].
...
- weighted log-likelihood: \(\ell(\beta,\tau)\) from Sectionâ€¯2.3  
...
This posterior is **logâ€‘concave in \(\tau\)**, enabling envelopeâ€‘based
acceptâ€“reject sampling in the Gamma regression case.
...
### 4.2 Independent Normalâ€“Gamma Prior
...
after each \(\tau\) step.
A **1000â€‘iteration burnâ€‘in** is followed by **1000 stored iterations**
(coefficient matrix `beta_out`, dispersion vector `disp_out`, and acceptâ€“reject
counts `iters_out`).
...
This decomposition enables a **tangentâ€“envelope acceptâ€“reject sampler**, in which:
...
â€¢ Gamma priors on the precision \( \tau \)  
â€¢ Joint Normalâ€“Gamma priors  
â€¢ Independent Normalâ€“Gamma priors  
â€¢ iid sampling via envelope methods  
â€¢ twoâ€‘block Gibbs sampling  

The logâ€‘concavity of the Gaussian likelihood in both \( \beta \) and \( \tau \)
ensures that all samplers are stable, efficient, and theoretically wellâ€‘behaved.
...
This closedâ€‘form marginal is what makes the Normalâ€“Gamma prior so computationally
attractive:
...
The dispersion (precision) parameter has a closedâ€‘form marginal posterior:
...
and \( \hat\beta_{\text{GLS}} \) is the generalized leastâ€‘squares estimator under
the prior precision.
...
All posterior samples are **independent**, making the conjugate Normalâ€“Gamma
...
**Chapter A07 â€” Accept-Reject Sampling for Gaussian Regression Models with
