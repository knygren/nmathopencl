...
\(\texttt{Sigma_0} = ((1-pwt)/pwt)\, G^{-1}\) (Zellner \(g\)-prior
...
\(a_{0} = n_{\mathrm{prior}}/2\) for `shape_df = "n_prior"`, so
...
**rate** to \(\texttt{rate} = (n_{\mathrm{prior}}/2)\, d\) with
\(d = \texttt{dispersion}\) returned by the function. In the notation above,
\(b_{0} = \texttt{rate}\).
...
Assume \(a_{0} = n_{\mathrm{prior}}/2\) (`shape_df = "n_prior"`). Then
