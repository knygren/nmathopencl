...
We build on the foundations from Chapters 05 and 06, especially the role of link functions, logâ€‘concavity, and prior specification.
...
- **Twoâ€‘column matrix:** \(\text{cbind(successes, failures)}\)
...
### 2.2 Weighted binomial logâ€‘likelihood

Using weights \(w_i = n_i\), the logâ€‘likelihood (up to constants) becomes
...
### 2.3 Exponentialâ€‘family representation
...
\mu_i = g^{-1}(\eta_i),
\]
the contribution of observation \(i\) to the logâ€‘likelihood can be written as
\[
\ell_i(\beta)
=
w_i\Big[
...
\Big],
\]
where \(w_i\) is the number of trials (or a userâ€‘supplied weight).
...
|---------------|---------|-------|
| **logit** (canonical) | \( \eta = \log(\mu/(1-\mu)) \) | Most common; canonical link; induces logâ€‘concavity in \(\eta\) |
| **probit** | \( \eta = \Phi^{-1}(\mu) \) | Based on the standard normal CDF; induces logâ€‘concavity in \(\eta\) |
| **cloglog** | \( \eta = \log[-\log(1-\mu)] \) | Asymmetric; useful for rare events; induces logâ€‘concavity in \(\eta\) |
| **cauchit** | \( \eta = \tan[\pi(\mu - 1/2)] \) | Heavyâ€‘tailed; does **not** preserve logâ€‘concavity in general |
| **identity** | \( \eta = \mu \) | Must ensure \(0 < \mu < 1\); does **not** preserve logâ€‘concavity in general |
...
Each link produces a different transformation \(g^{-1}(\eta)\) and therefore a different expression for the logâ€‘likelihood and its derivatives. For the logit, probit, and cloglog links, the resulting logâ€‘likelihood is known to be logâ€‘concave in the linear predictor \(\eta\), which is crucial for stable envelope construction and acceptâ€“reject sampling in *glmbayes*.
...
- a Zellnerâ€‘type gâ€‘prior [@GriffinBrown2010]  
...
It is symmetric and interpretable in terms of **logâ€‘odds**.
...
The DIC often favors probit slightly for smooth Sâ€‘shaped curves (as seen in Chapter 04).

---

## 6. Complementary Log-Log (cloglog) Link: Likelihood, Prior, Posterior

The complementary logâ€“log link is often used for asymmetric response curves and hazardâ€‘type interpretations [@McCullagh1989; @Agresti2015].
...
The cloglog model often fits poorly for symmetric Sâ€‘shaped curves (as shown in Chapter 04), but it is valuable for:

- rare events  
- survivalâ€‘type interpretations  
...
- **cloglog** is best when the response is asymmetric or hazardâ€‘like  
...
Binomial GLMs are a core component of the glmbayes package. Their logâ€‘concave likelihoods make them ideal for the envelopeâ€‘based acceptâ€‘reject sampler, and the familiar link functions allow analysts to choose models that match the scientific context [@McCullagh1989; @Gelman2013].
