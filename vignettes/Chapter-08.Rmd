...
The Poisson likelihood is logâ€‘concave under the log link, making it an ideal setting for envelopeâ€‘based Bayesian sampling [@Nygren2006].  
...
We use the wellâ€‘known randomized controlled trial dataset of [@Dobson1990], which appears in many GLM textbooks and is included in the base R documentation for glm().  
...
### 2.1 Weighted Poisson Logâ€‘Likelihood

Up to constants, the weighted logâ€‘likelihood is
...
and the logâ€‘likelihood simplifies to
...
### 2.2 Exponentialâ€‘Family Representation
...
This identity ensures that the Poisson logâ€‘likelihood is **globally logâ€‘concave in the linear predictor**, which is ideal for envelope construction and acceptâ€“reject sampling in *glmbayes*.
...
- yields a concave logâ€‘likelihood in \(\eta_i\)  
- ensures stable IRLS updates in classical GLMs  
- supports efficient envelopeâ€‘based iid sampling in *glmbayes*

Noncanonical links (identity, squareâ€‘root) do **not** preserve logâ€‘concavity and are not supported in `glmb()`.


### 2.4 Likelihood, Prior, and Posterior (Normal Prior on beta)

With the log link, the weighted logâ€‘likelihood is
...
Combining likelihood and prior yields the logâ€‘posterior:
...
Because both terms are concave in \(\beta\), the posterior is **logâ€‘concave**, enabling efficient iid sampling via the envelopeâ€‘based acceptâ€“reject sampler implemented in *glmbayes*.

## 3. Data Setup

We reproduce Dobsonâ€™s example exactly.  
...
The table shows the observed counts for each treatmentâ€“outcome combination.
...
- **Coefficient estimates** on the logâ€‘scale  
...
The recommended workflow uses Prior_Setup(), which constructs a Zellnerâ€‘type gâ€‘prior aligned with the model matrix.
...
- the Poisson likelihood is strongly logâ€‘concave  
...
- Poisson GLMs with log links are ideal for envelopeâ€‘based Bayesian sampling  
...
- overdispersed quasiâ€‘Poisson models  
- informative priors  
- prediction and posterior predictive checks  
- GPUâ€‘accelerated Poisson models for large datasets
