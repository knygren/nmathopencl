...
where 300000 ms ≈ 5 minutes. The calibration run uses `parallelFor(0, m_stage, worker)` to measure per-draw time empirically.
...
If the estimated time for either phase exceeds 5 minutes:

- **RSS:** `"Estimated minimization exceeds 5 minutes. Continue? [y/N]: "`
- **UB2:** `"Estimated UB2 minimization exceeds 5 minutes. Continue? [y/N]: "`

In non-interactive sessions, the run proceeds automatically.

This is parallel **envelope construction**, not sampling, but it uses the same safeguard pattern because it, too, cannot be interrupted once started.
