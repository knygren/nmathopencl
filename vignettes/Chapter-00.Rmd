...
| Chapter 02 | Adding `USE_OPENCL` and `has_opencl()` to your own package (developer porting guide) |
| Chapter 03 | Structure of nmath kernel programs: the four-layer assembly model |
| Chapter 04 | Kernels, kernel runners, and kernel wrappers: roles and interaction |
| Chapter 05 | The `nmath` OpenCL library (`inst/cl/nmath/`) |
| Chapter 06 | Kernels: writing and using `__kernel` functions |
| Chapter 07 | The `nmathopencl` R API: distribution functions on the GPU |
| Chapter 08 | Kernel loading: `load_kernel_source` and `load_kernel_library` |
| Chapter 09 | Internal generic OpenCL kernel runners (C++ infrastructure) |
