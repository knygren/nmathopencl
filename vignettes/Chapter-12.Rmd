...
This chapter describes how to enable GPU acceleration in `glmbayes` using OpenCL. GPU acceleration can dramatically reduce computation time for large envelope models, especially when working with highâ€‘dimensional predictors or repeated model fits. OpenCL is used because it is vendorâ€‘neutral and works across NVIDIA, AMD, and Intel hardware. CPUâ€‘only execution remains fully supported, but may be significantly slower for large models.
...
- Ubuntu/Debian:
  ...
  (If you need the latest R, add the CRAN repo first â€” see â€œInstalling the latest R on Ubuntu/Debianâ€ below.)
...
Ubuntuâ€™s default repositories often contain older R versions. To install the current CRAN release:
...
sudo add-apt-repository "deb https://CRAN.R-project.org/bin/linux/ubuntu/ jammy-cran40/"
sudo apt-get update
sudo apt-get install build-essential r-base-dev
...
This is because the packageâ€™s configure script uses GCC to detect system include and library paths 
for OpenCL.
...
Most GPU drivers provide only the vendorâ€‘specific OpenCL runtime, not the headers or the development symlink. The following subsections describe how to install the required OpenCL components on Windows, Linux, and macOS.
...
- **Khronos OpenCL Headers (headerâ€‘only download)**  
...
2. OpenCL header files (compileâ€‘time)  
...
For AMD, you must choose the correct OpenCL stack â€” see Appendix A.
...
Having headers and `libOpenCL` installed does **not** guarantee that a **vendor** OpenCL implementation registers a **platform**. The `glmbayes` configure script and `diagnose_glmbayes()` both probe for a usable platform (e.g. `clGetPlatformIDs()`). On Linuxâ€”especially **remote GPU VMs**â€”it helps to check this **outside R** first.
...
You should see **`Number of platforms` >= 1** and at least one **GPU** device (for NVIDIA, typically under the NVIDIA platform). If **`Number of platforms` is 0**, the ICD loader is present but **no vendor runtime** is visibleâ€”install your vendorâ€™s OpenCL ICD (on Ubuntu/Debian with NVIDIA GPUs, `sudo apt-get install nvidia-opencl-icd` is often required in minimal images) and run **`clinfo`** again.
...
## **3. Install glmbayes from source via Râ€‘universe**

Because CRAN and Râ€‘universe do not build packages with OpenCL support, installing from source is required to enable GPU acceleration.
...
On **Linux**, if you have shell access (including **Jupyter Terminal** on a cloud instance), running **`clinfo`** before opening R is recommended: confirm **`Number of platforms` >= 1** (see Â§2.2.5). That catches missing vendor ICDs early, independent of the R session.
...
- **GPU: NVIDIA**  
  ...
  (May show Intel, AMD, or â€œCPUâ€ depending on the system.)
...
- **[OK] OpenCL fully available (headers + runtime)**  
  Confirms that both compileâ€‘time and runâ€‘time OpenCL components are present.
...
The chunks below are **not executed** during vignette builds (runtime). They mirror a large `glmb` CPU vs OpenCL comparison; for an interactive GPU check use **`example(Cleveland)`** (Â§6 above).
...
knitr::include_graphics(
  system.file("extdata", "cleveland_non_opencl_output_01.png", package = "nmathopencl")
)
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics(
  system.file("extdata", "cleveland_opencl_output_01.png", package = "nmathopencl")
)
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics(
  system.file("extdata", "cleveland_summary_output_01.png", package = "nmathopencl")
)
knitr::include_graphics(
  system.file("extdata", "cleveland_summary_output_02.png", package = "nmathopencl")
)
```
...
- ROCmâ€™s optimized OpenCL implementation  
...
- **AMDGPUâ€‘PRO OpenCL**: legacy and not recommended.
