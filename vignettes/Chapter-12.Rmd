...
AMD's Windows driver package includes the OpenCL runtime and ICD components automatically. No additional installation or PATH configuration is required. If OpenCL is not detected, updating to the latest AMD Software (Adrenalin Edition) typically resolves the issue. Use `diagnose_glmbayes()` (**nmathopencl** or **glmbayes**, each reporting that package's compile-time status) or **opencltools** host probes to confirm AMD OpenCL components.
...
Having headers and `libOpenCL` installed does **not** guarantee that a **vendor** OpenCL implementation registers a **platform**. Source builds of **glmbayes** / **nmathopencl** and **`opencltools::verify_opencl_runtime()`** probe for a usable platform (e.g. `clGetPlatformIDs()`). On Linux---especially **remote GPU VMs**---it helps to check this **outside R** first.
...
**Note:** `clinfo` is a system diagnostic, not part of R. It complements `diagnose_glmbayes()` and **opencltools** runtime probes when debugging **cloud or container** setups where the NVIDIA driver looks fine (`nvidia-smi` works) but OpenCL still reports no platform.
...
On **Linux**, if you have shell access (including **Jupyter Terminal** on a cloud instance), running **`clinfo`** before opening R is recommended: confirm **`Number of platforms` >= 1** (see Section 2.2.5). That catches missing vendor ICDs early, independent of the R session.

```r
has_opencl()
```

- Returns `TRUE` if **nmathopencl** was built with OpenCL compile support.
- Returns `FALSE` on typical CRAN/prebuilt binaries without `USE_OPENCL`.
...
This **nmathopencl** report combines **opencltools** host/runtime checks with
this package's compile-time status. For opencltools-only inventory (no
nmathopencl compile line), use `opencltools::diagnose_glmbayes()` after
`library(opencltools)`. Lower-level probes include
`opencltools::detect_environment_and_gpus()`,
`opencltools::detect_compute_runtimes()`, and
`opencltools::verify_opencl_runtime()`.
...
The chunks below are **not executed** during vignette builds (runtime). They mirror a large `glmb` CPU vs OpenCL comparison; for an interactive GPU check use **`example(Cleveland)`** (A?6 above).
