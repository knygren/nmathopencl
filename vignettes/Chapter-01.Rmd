...
The **glmb()** family of functions provides fully Bayesian analogues to **glm()** and its standard methods. This vignette demonstrates how to set up and call **glmb()**, compares its syntax and output to **glm()**, and highlights the additional arguments required for prior specification. A specialized **lmb()** function for Bayesian linear regression is bundled in the package and will be explored in Chapter 2. Separate vignettes cover all complementary methods for both **lmb()** and **glmb()**.



# 2. Preparing a dataframe

We begin by reproducing Dobson’s (1990) randomized controlled trial example, which is the standard demo for **glm()**. Comparing **glmb()** output to this familiar dataset will make parallels clear. The data frame contains treatment groups, outcomes, and observed counts:
...
| 4.2.3 Expected Residual Deviance           | 10.3 Bayesian Generalized Linear Models                              | Explains computation of \(\bar D\), the posterior-mean deviance, and its use in posterior predictive checks (see p. XXX).                                     |
...
# 7. Redundant - To be deleted or moved elsewhere



# 7B Summarizing the output
Let's take a closer look at the outputs of the summary functions.


## 7.1 Summary view of the glm output
...
## 7.2 Summary view of the glmb output
