...
notes also appear in **Chapter 12**.
...
**`has_opencl()`** returns `FALSE` and every `*_opencl` wrapper silently uses
its `stats::` fallback.
...
attach message compares `has_opencl()` (nmathopencl) and
...
If attach is silent and `has_opencl()` returns `TRUE`, you are done --- skip
...
When `opencltools::has_opencl()` and `has_opencl()` are both `FALSE`, the host
stack or compile flags are missing for both packages.

```{r, eval = FALSE}
opencltools::has_opencl()   # opencltools build flag
has_opencl()                # nmathopencl build flag
diagnose_glmbayes()         # combined report (host checks + nmathopencl compile line)
```
...
For a host-only report without the nmathopencl compile-time line, use
`opencltools::diagnose_glmbayes()` directly.
...
When `opencltools::has_opencl()` is `TRUE` but `has_opencl()` is `FALSE`, the
...
# Full diagnostic report (opencltools host checks + nmathopencl compile line)
diagnose_glmbayes()
```

`diagnose_glmbayes()` (exported from **nmathopencl**) runs a layered check:
steps 1--7 use **opencltools** (environment, GPU vendors, drivers, OpenCL
headers and ICD, Linux/WSL runtime probe, PATH / `LD_LIBRARY_PATH`); the
final step reports `has_opencl()` for this **nmathopencl** build. On Windows,
the Linux/WSL runtime probe is skipped; rely on driver and ICD checks instead.

### Validating GPU calls with the exported wrappers

Once `has_opencl()` is `TRUE`, the exported `*_opencl` functions dispatch to
...
expected behaviour, not a bug. The real-world acceleration from using the
...
adding `USE_OPENCL` and `has_opencl()` to your own downstream package so
