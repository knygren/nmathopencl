...
1. **OPENCL.cl** â€“ Global configuration: extensions (`cl_khr_fp64`, `cl_khr_printf`), IEEE constants (`ML_NAN`, `ML_POSINF`), feature detection for `expm1`/`log1p`, and utility macros.
2. **rmath** â€“ Mathematical constants (M_E, M_PI, etc.) and distribution function declarations.
3. **dpq** â€“ R-style density/CDF macros (`R_D__0`, `R_DT_val`, etc.) used in give_log/lower_tail logic.
4. **nmath** â€“ Ported numerical routines (bd0, stirlerr, lgamma, dbinom, dpois, pnorm, etc.).
5. **Kernel file** â€“ The model-specific kernel (e.g., `f2_f3_binomial_logit.cl`) that computes f2 and f3 for each grid point.

### 3.2 load_kernel_source and load_kernel_library

- **`load_kernel_source(relative_path)`** â€“ Loads a single `.cl` file from `inst/cl/` via `system.file("cl", relative_path, package = "nmathopencl")`.
- **`load_kernel_library(subdir)`** â€“ Loads all `.cl` files in a subdirectory (e.g., `"nmath"`, `"rmath"`, `"dpq"`), parses `@provides` and `@depends` annotations, performs a **dependency-aware topological sort**, and concatenates the files so that dependents appear after their dependencies.
...
1. **Work-item mapping** â€“ `int j = get_global_id(0)`; one work item per grid point \(j\).
2. **Prior term** â€“ Compute \(P(\beta_j - \mu)\) and the quadratic form \(\tfrac{1}{2}(\beta_j - \mu)' P (\beta_j - \mu)\), accumulate into `qf[j]`.
3. **Prior gradient** â€“ Initialize local gradient with \(P(\beta_j - \mu)\).
4. **Data loop** â€“ For each observation \(i\), compute linear predictor, link, likelihood contribution, and gradient contribution. Use ported functions (`dbinom_raw`, `dpois_raw`, `lgamma`, `pnorm5`, etc.) as needed.
5. **Write outputs** â€“ `qf[j]` = negative log posterior; `grad[k*m1 + j]` = gradient (column-major layout).
...
1. **Input flattening** â€“ Convert R matrices/vectors (x, b, mu, P, alpha, y, wt) to contiguous C++ vectors in the layout expected by the runner.
2. **Kernel selection** â€“ Map `(family, link)` to `kernel_name` and `kernel_file` (e.g., binomial + logit â†’ `f2_f3_binomial_logit`, `src/f2_f3_binomial_logit.cl`).
3. **Program assembly** â€“ Load OPENCL.cl, rmath, nmath, dpq, and the kernel file; concatenate into `all_src`.
4. **Runner call** â€“ Invoke `f2_f3_kernel_runner(all_src, kernel_name, l1, l2, m1, ...)` with flattened inputs and output buffers.
5. **Output reshaping** â€“ Copy `qf_flat` into an R vector; wrap `grad_flat` as an Armadillo matrix (m1 Ã— l2) for return.
...
1. **Platform and device** â€“ `clGetPlatformIDs`, `clGetDeviceIDs` (CL_DEVICE_TYPE_DEFAULT).
2. **Context and queue** â€“ `clCreateContext`, `clCreateCommandQueueWithProperties`.
3. **Program** â€“ `clCreateProgramWithSource` with the concatenated string, `clBuildProgram`.
4. **Kernel** â€“ `clCreateKernel` with the kernel name.
5. **Buffers** â€“ Create read-only buffers for X, B, mu, P, alpha, y, wt; write-only for qf and grad. Copy host data for inputs.
6. **Launch** â€“ `clEnqueueNDRangeKernel` with `global = m1` (one work item per grid point).
7. **Readback** â€“ `clEnqueueReadBuffer` for qf and grad.
8. **Sanity check** â€“ If both outputs are all zeros, throw (likely kernel failure).
9. **Cleanup** â€“ Release buffers, kernel, program, queue, context.
...
- **Warm-up** â€“ Run `f2_f3_opencl` on a small grid slice.
- **Calibration** â€“ Run on slices of ~1% and ~2% of the grid to estimate fixed cost and per-grid cost.
- **Refined estimate** â€“ Compute `refined_est_total_sec = per_grid_sec Ã— m1`.
- **5-minute safeguard** â€“ If `refined_est_total_sec > 300`:
...
- **Chapter A05** â€“ Simulation pipeline (optimization, standardization, envelope build, sampling)
- **Chapter A08** â€“ Envelope-related functions
- **Chapter A09** â€“ Parallel sampling (pilot pattern, 5-minute safeguard)
- **Chapter 12** â€“ GPU acceleration: installation, `has_opencl`, `diagnose_glmbayes`
- **`?EnvelopeBuild`**, **`?EnvelopeEval`** â€“ use_opencl argument
- **`?load_kernel_library`**, **`?load_kernel_source`** â€“ Program assembly
- **`?glmb`**, **`?rglmb`** â€“ use_opencl argument
