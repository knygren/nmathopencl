...
- **`load_kernel_source(relative_path)`** --- Loads a single `.cl` file from `inst/cl/` via `system.file("cl", relative_path, package = "nmathopencl")`.
...
Use `has_opencl()` to check **nmathopencl** compile-time support and
`diagnose_glmbayes()` for a combined report (opencltools host checks plus this
build's OpenCL flag). Host-only diagnostics live in **`opencltools`**. See
Chapter 12 for platform-specific installation (Windows, Linux, macOS) and AMD
ROCm notes.
...
- **Chapter 12** --- GPU acceleration: installation, `has_opencl`, `diagnose_glmbayes`, **opencltools** host probes
