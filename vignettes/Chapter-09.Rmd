...
Gamma regression models are used when the response variable is strictly positive and rightâ€‘skewed.  
...
Gamma regression is a standard GLM for positive, rightâ€‘skewed responses [@NelderWedderburn1972; @McCullagh1989; @Agresti2015].
...
This property is essential for stable envelope construction and acceptâ€“reject sampling in *glmbayes*.
...
Models with **estimated dispersion** or **non-log links** are covered in Chapterâ€¯11.
...
Because both terms are concave in \(\beta\), the posterior is **log-concave**, enabling efficient iid sampling via the envelope-based acceptâ€“reject sampler implemented in *glmbayes*.
...
- it preserves logâ€‘concavity of the likelihood
...
- regression coefficients on the log meanâ€‘cost scale  
- standard errors and zâ€‘statistics  
- deviance and AIC  
- an estimated dispersion parameter (phi)

Under the log link, exp(\(\beta_{j}\)) can be interpreted as a multiplicative factor on the expected cost ratio for a oneâ€‘unit change in the corresponding covariate (or a change in factor level).
...
Next we use `Prior_Setup()` to construct a Zellnerâ€‘type gâ€‘prior [@GriffinBrown2010] for the coefficients, aligned with the Gamma logâ€‘link model and weighting structure:
...
- exp(\(\beta_{j}\)) is the multiplicative change in expected cost for a oneâ€‘unit change in the covariate (or for a change from baseline to a given factor level).
...
- direct comparison of logâ€‘likelihood based quantities,  
- a clean assessment of how the prior affects coefficient estimates.

If desired, a more advanced model can estimate dispersion by switching to a pfamily such as dNormal_Gamma, but that is covered in more detail in the dispersionâ€‘focused vignettes.
...
- used Prior_Setup() to construct a default Zellnerâ€‘type prior,  
...
- and specialized acceptâ€‘reject schemes for dispersion parameters.
