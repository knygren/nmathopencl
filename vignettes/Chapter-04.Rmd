---
title: "Chapter 04: Kernels, Kernel Runners, and Kernel Wrappers"
author: "Kjell Nygren"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Chapter 04: Kernels, Kernel Runners, and Kernel Wrappers}
...
2. A concrete **example** drawn from the `ex_glmbayes` component of
   ...
   a grid of candidate coefficient vectors.  This is labelled clearly as an
   example throughout.
...
The work-items are organised internally into *work-groups* for scheduling
...
3. **Conditionally dispatches** to the runner if `USE_OPENCL` was defined at
   build time.  If not, the function returns the zero-initialised outputs.
...
```cpp
  ...
  Rcpp::NumericVector qf(m1);   // zero-initialised; returned as-is on CPU-only build
```

`std::vector<double>` zero-initialises its elements by default.  If
...
which happens after `Rcpp::List::create` serialises the result — so the
...
OpenCL C source, optimises it, and produces GPU-native machine code specific
...
allocates uninitialised device memory that the kernel will write into.  The
...
│     outputs: WRITE_ONLY, nullptr → uninitialised device memory  │
